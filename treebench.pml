...
fun countLeaves (t: tree) : Int.int =
   case t of
      Leaf n        => 1
...
(*    let *)
(*       fun graftTree (root: Int.int, power: int): tree = *)
(*          if power = 0 *)
(*             then Leaf root *)
...
fun benchmarks_sum (power: int, trials: int): (microreal * microreal) =
   let
      val _ = print "Benchmarking"
      fun computeTimes (its: int, t: tree): Int.int list =
         if its = 0
            then []
            else
	      let val _ = print "." in
...
fun mkRun optToStr f = let
     ...
     val e = Time.now ()
     val _ = (case optToStr
       of SOME tos => Print.printLn ("BATCHTIME: " ^ tos (e-b))
        | NONE => ()
       (* end case *))
     in
       ans
...
val _ = run (fn () => putStrLn "Blah ... test")
